...
@section('content')
 <div class="carousel-inner">
   <div class="carousel-item active">
    <img src="{{ asset('frontend/img/event.png') }}" class="d-block w-100 custom-img" alt="...">
    <div class="carousel-caption">
     <h1 class="fw-bold text-uppercase custom-heading">acara</h1>
    </div>
    </div>
 </div>
...
<div class="card-group container">
  ...
  <div class="col-md-3 mb-4">
    <div class="card mx-2 shadow rounded">
      ...
      <div class="card-body">
        <h6 class="card-title text-warning">{{ $row->tanggal_mulai }} <span class="text-dark"><-></span> {{ $row->tanggal_berakhir }}</h6>
        <h4 class="card-title">{{ $row->title }}</h4>
      </div>
      <div class="card-footer bg-transparent">
        <p class="card-text">{{ $row->lokasi }}</p>
      </div>
      <a class="btn btn-dark" href="{{ route('detailAcara', ['id' => $row->id]) }}" role="button">SELENGKAPNYA</a>
    </div>
  </div>
  @endforeach
</div>
